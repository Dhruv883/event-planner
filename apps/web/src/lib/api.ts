...
export async function votePoll(
  ...
  await apiRequest<null>(
    `/api/events/${encodeURIComponent(eventId)}/polls/${encodeURIComponent(pollId)}/vote`,
    { method: "POST", data: payload }
  );
}
...
export async function updatePoll(
  ...
  payload: Partial<{
    ...
    settings: Partial<{
      ...
      voterPermission: "ALL_ATTENDEES" | "ACCEPTED_ATTENDEES" | "HOSTS_ONLY";
      resultVisibility:
        | "VISIBLE_TO_ALL"
  ...
  const res = await apiRequest<{ data: { id: string } }>(
    `/api/events/${encodeURIComponent(eventId)}/polls/${encodeURIComponent(pollId)}`,
